...
public final class BubbleTextView extends TextView {
    ...
    public BubbleTextView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            typed = context.obtainStyledAttributes(attrs, R.styleable.BubbleTextView);
            radiusSize = typed.getDimension(R.styleable.BubbleTextView_btv_radius_size, radiusSize);
            shadowRadiusSize = typed.getDimension(R.styleable.BubbleTextView_btv_shadow_radius_size, shadowRadiusSize);

            strokeSize = typed.getDimension(R.styleable.BubbleTextView_btv_stroke_size, strokeSize);
            arrowWidth = typed.getDimension(R.styleable.BubbleTextView_btv_arrow_width, arrowWidth);
            arrowHeight = typed.getDimension(R.styleable.BubbleTextView_btv_arrow_height, arrowHeight);
            arrowMargin = typed.getDimension(R.styleable.BubbleTextView_btv_arrow_margin_top, arrowMargin);

            strokeColorNormal = typed.getColor(R.styleable.BubbleTextView_btv_stroke_color_normal, strokeColorNormal);
            strokeColorSelect = typed.getColor(R.styleable.BubbleTextView_btv_stroke_color_press, strokeColorSelect);

            isArrowRight = typed.getBoolean(R.styleable.BubbleTextView_btv_arrow_right, isArrowRight);
            isArrowLeft = typed.getBoolean(R.styleable.BubbleTextView_btv_arrow_left, isArrowLeft);
    ...
    public void draw(Canvas canvas) {
        ...
        if (isArrowRight) {
            ...
            drawRightBackground(canvas, width, height);
        } else if (isArrowLeft) {
            final float width = getWidth();
            final float height = getHeight();
            drawLeftBorder(canvas, width, height);
        }
    ...
    private final void drawRightBorder(Canvas canvas, float width, float height) {
        // step0
        mPath.reset();
        // step1
    ...
    private final void drawRightBackground(Canvas canvas, float width, float height) {
        // step0
        mPath.reset();
        // step1
        mPath.moveTo(radiusSize + strokeSize * 0.5f, strokeSize);
        // step2
        mPath.lineTo(width - radiusSize - arrowWidth - strokeSize * 0.5f, strokeSize);
        // step3
        mRectF.set(width - 2 * radiusSize - arrowWidth, strokeSize, width - strokeSize - arrowWidth, 2 * radiusSize);
        mPath.arcTo(mRectF, 270, 90, false);
        // step4
        mPath.lineTo(width - strokeSize - arrowWidth, arrowMargin);
        // step4
        mPath.lineTo(width - strokeSize, arrowMargin + 0.5f * arrowHeight);
        // step5
        mPath.lineTo(width - strokeSize - arrowWidth, radiusSize + arrowHeight + arrowMargin);
        // step6
        mPath.lineTo(width - strokeSize - arrowWidth, height - radiusSize - strokeSize * 0.5f);
        // step7
        mRectF.set(width - 2 * radiusSize - arrowWidth, height - 2 * radiusSize, width - strokeSize - arrowWidth, height - strokeSize);
        mPath.arcTo(mRectF, 0, 90, false);
        // step8
        mPath.lineTo(radiusSize + strokeSize * 0.5f, height - strokeSize);
        // step9
        mRectF.set(strokeSize, height - 2 * radiusSize, 2 * radiusSize, height - strokeSize);
        mPath.arcTo(mRectF, 90, 90, false);
        // step10
        mPath.lineTo(strokeSize, radiusSize + strokeSize * 0.5f);
        // step11
        mRectF.set(strokeSize, strokeSize, 2 * radiusSize, 2 * radiusSize);
        mPath.arcTo(mRectF, 180, 90, false);
        // step12
        mPath.close();
        // step13
        ...
        mPaint.setStrokeCap(Paint.Cap.ROUND);
        mPaint.setFakeBoldText(true);
        mPaint.setColor(!isTouch ? Color.RED : Color.BLUE);
        mPaint.setStrokeWidth(strokeSize);
        mPaint.setTextSize(0f);
        mPaint.setTextAlign(Paint.Align.CENTER);
    ...
    private final void drawLeftBorder(Canvas canvas, float width, float height) {
        // step0
        mPath.reset();
        // step1
